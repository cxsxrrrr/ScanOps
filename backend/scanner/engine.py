            ...
            with socket.create_connection((hostname, 443), timeout=self.timeout) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    if cipher:
                        ...
                        if WEAK_CIPHER_PATTERN.search(cipher_name) or any(weak in cipher_name.upper() for weak in weak_ciphers):
                            self.findings.append({
                                ...
                                'recommendation': (
                                    'Disable RC4, DES/3DES, MD5, NULL, RC2, IDEA, and export-grade '
                                    'ciphers. Configure the server to use only modern AEAD ciphers (AES-GCM, '
                                    'ChaCha20-Poly1305).'
                                ),
